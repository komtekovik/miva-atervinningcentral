<script lang="ts">
    ...
    const mapComponents = {
        må: { base: MapBaseMa, containers: MapContainersMa, labels: MapLabelsMa },
        bjästa: { base: MapBaseBjasta, containers: MapContainersBjasta, labels: MapLabelsBjasta },
        björna: { base: MapBaseBjorna, containers: MapContainersBjorna, labels: MapLabelsBjorna },
        husum: { base: MapBaseHusum, containers: MapContainersHusum, labels: MapLabelsHusum },
        bredbyn: { base: MapBaseBredbyn, containers: MapContainersBredbyn, labels: MapLabelsBredbyn }
    };
    ...
    onMount(() => {
        ...
        if (viewport) {
            ...
            return () => viewport.removeEventListener('resize', resizeHandler);
        }
    });
...
<style>
    ...
    .map-container {
        display: grid;
        width: min(100dvw, 120dvh * (2481 / 3508));
        height: min(120dvh, 100dvw * (3508 / 2481));
        background: white;
